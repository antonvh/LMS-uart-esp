...
class LPF2(object):
     def __init__(self, uartChannel, txPin, rxPin, modes = defaultModes, type = WeDo_Ultrasonic, timer = 4, freq = 5):
          self.txPin = txPin
          self.rxPin = rxPin
          self.uart = machine.UART(uartChannel,baudrate=2400,rx=rxPin,tx=txPin,timeout=5)
          self.txTimer = timer
          self.modes = modes
          self.current_mode = 0
          self.type = type
...
# -----  Init and close
     ...
     def init(self):
          ...
          utime.sleep_ms(500)
          self.tx.value(1)
          self.uart=machine.UART(2,baudrate=115200,rx=self.rxPin,tx=self.txPin)
          self.writeIt(b'\x00')
...
class ESP_LPF2(LPF2):
    def init(self):
          ...
          utime.sleep_ms(500)
          self.tx.value(1)
          self.uart=machine.UART(2,baudrate=2400,rx=self.rxPin,tx=self.txPin)
          self.writeIt(b'\x00')
