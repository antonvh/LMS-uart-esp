    ...
    def execute_command(self, command, *value):
        ...
        if command in self.commands:
            ...
            if command[-3:]!='ack':# discard any ack from other command
                self.send_command('err','s','nok')

    def wait_and_execute(self):
        command,value=self.wait_and_receive()
        if type(value)==tuple:
            self.execute_command(command,*value)
        else: # value is just a single value
            self.execute_command(command,value)

    def check_and_execute(self):
        command, value = self.receive_command()
        if command:
            if command[-3:] not in ['err', 'ack']:
                self.execute_command(command, *value)
